...
## Future: Команда `fix-names` (нормализация мультиязычных имён)

### Проблема

В Geni все имена записаны в locale `en`, даже если они на кириллице:
```json
{
  "names": {
    "en": {"first_name": "Иван", "last_name": "Иванов"}  // Неправильно!
  }
}
```
...
Исправить локали и добавить транслитерацию:
```json
{
  "names": {
    "ru": {"first_name": "Иван", "last_name": "Иванов"},
...
### Логика определения скрипта

```csharp
bool IsCyrillic(string text)
{
    // Unicode блок Cyrillic: U+0400–U+04FF
    return text.Any(c => c >= '\u0400' && c <= '\u04FF');
}
...
```
Для каждого профиля:
1. Получить текущие names[en]
2. Для каждого поля (first_name, last_name, maiden_name...):
   a. Если IsCyrillic(value):
      - Переместить в names[ru]
      - Создать транслит в names[en]
   b. Если IsLatin(value):
      - Оставить в names[en]
      - Создать транслит в names[ru] (опционально)
3. Вызвать UpdateProfileAsync с новыми names
```

### Транслитерация ru → en

Простая таблица (можно расширить):
```csharp
static readonly Dictionary<char, string> CyrToLat = new()
{
    ...
    {'ш', "sh"}, {'щ', "shch"}, {'ъ', ""}, {'ы', "y"}, {'ь', ""},
    {'э', "e"}, {'ю', "yu"}, {'я', "ya"},
    // Заглавные аналогично...
};
```

**Альтернатива:** использовать `givenname_similar_names.csv` для поиска известных вариантов имён.

### CLI

```bash
gedsync fix-names \
  --anchor <geni-profile-id> \
  --max-depth 10 \
  --dry-run \
  --verbose
```
...
**Параметры:**
- `--anchor` — Geni profile ID для BFS обхода
- `--max-depth` — глубина обхода (default: 10)
- `--dry-run` — только показать что будет изменено
- `--only-cyrillic` — исправлять только кириллицу в en
- `--add-translits` — добавлять транслиты (default: true)

### Ограничения v1

- Только `ru` и `en` локали
- Прибалтийские/другие латиничные языки — оставить как есть
- Смешанные имена (кириллица + латиница) — пропускать с warning

### Приоритет

Реализовать **после** `update` и `add`, как отдельную утилиту. Может быть полезна для массовой очистки данных перед синхронизацией.
